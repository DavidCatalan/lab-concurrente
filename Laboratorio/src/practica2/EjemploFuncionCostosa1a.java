...
class EjemploFuncionCostosa1a {
	...
	public static void main(String args[]) {
		...
		System.out
				.println("Tiempo secuencial (seg.):                    " + tt);
		...
		System.out
				.println("Inicio del programa concurrente con distribución cíclica.");
		...
		tt = ((double) (t2 - t1)) / 1.0e9;
		System.out
				.println("Tiempo concurrente cíclico (seg.):                    "
						+ tt);
		...
		System.out
				.println("Inicio del programa concurrente con distribución por bloques.");
		...
		tt = ((double) (t2 - t1)) / 1.0e9;
		System.out
				.println("Tiempo concurrente por bloques (seg.):                    "
						+ tt);
	...
	static void imprimeResultado(double vectorX[], double vectorY[]) {
		for (int i = 0; i < Math.min(vectorX.length, vectorY.length); i++) {
			System.out.println("  i: " + i + "  x: " + vectorX[i] + "  y: "
					+ vectorY[i]);
		}
	}
}
...
class HebraCiclica extends Thread {
	...
	public HebraCiclica(int miId, int numHebras, double[] vectorX,
			double[] vectorY) {
	...
	public void run() {
		for (int i = miId; i < vectorX.length; i += numHebras) {
			vectorY[i] = Math.sin(Math.exp(-vectorX[i])
					+ Math.log(1 + vectorX[i]));
		}
	}
}
...
class HebraBloques extends Thread {
	...
	public HebraBloques(int miId, int numHebras, double[] vectorX,
			double[] vectorY) {
		this.vectorX = vectorX;
		this.vectorY = vectorY;
		this.miId = miId;
