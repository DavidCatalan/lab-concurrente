...
// ============================================================================
  ...
  public static void main( String args[] ) {
    ...
    for(int i = 0; i < numHebras; i++) {
    	vHebras[i] = new HebraCiclicaSencilla(i, numHebras, vectorX, vectorY);
    	vHebras[i].start();
    }
...
//Fin parte concurrente ciclica---------
 ...
 //Parte concurrente por bloques---------
  ...
  for(int i = 0; i < numHebras; i++) {
  	vHebras[i] = new HebraBloquesSencilla(i, numHebras, vectorX, vectorY);
  	vHebras[i].start();
  }
...
class HebraCiclicaSencilla extends Thread {
	double[] vectorX, vectorY;
	int miId, numHebras;
	
	public HebraCiclicaSencilla(int miId, int numHebras, double[] vectorX, double[] vectorY) {
	...
	public void run() {
		for(int i = miId; i < vectorX.length; i += numHebras) {
			vectorY[ i ] = 2.5 * vectorX[i];
		}
	}
}

class HebraBloquesSencilla extends Thread{
	double[] vectorX,vectorY;
	int miId,numHebras;
	
	public HebraBloquesSencilla (int miId, int numHebras, double[] vectorX, double[] vectorY) {
		this.vectorX = vectorX;
		this.vectorY = vectorY;
		this.miId = miId;
		this.numHebras = numHebras;
